 ...
 */
public final class WindowDecor {
    public static final int FRAME_TOP_H = 23;
    public static final int FRAME_BOTTOM_H = 26;
    public static final int FRAME_SIDE_W = 4;
    ...
    public float contentX(float wx) {
        return wx + FRAME_SIDE_W;
    }

    public float contentY(float wy) {
        return wy + FRAME_TOP_H;
    }

    public float contentW(float ww) {
        return Math.max(0f, ww - 2f * FRAME_SIDE_W);
    }

    public float contentH(float wh) {
        return Math.max(0f, wh - FRAME_TOP_H - FRAME_BOTTOM_H);
    }

    public void draw(float wx, float wy, float ww, float wh) {
        float c = TOP_CORNER;
        float b = FRAME_BOTTOM_H;
        float s = FRAME_SIDE_W;
        float t = FRAME_TOP_H;
        float innerH = Math.max(0f, wh - t - b);
        float topMidW = Math.max(0f, ww - 2f * c);
        float botMidW = topMidW;
        ...
        blit(middleCenter, wx + s, wy + t, Math.max(0f, ww - 2f * s), innerH);
    ...
    private void blit(Tile r, float sx, float sy, float sw, float sh) {
        if (sw <= 0f || sh <= 0f) {
            return;
        }
    ...
    private static final int TOP_CORNER = 23;
    private static final int BOTTOM_CORNER_W = 23;

    static final class WindowDecorJson {
        String image;
        AtlasJson atlas;
        TilesJson tiles;
    }
    ...
    public static WindowDecor load(File jsonFile, Gson gson) {
        ...
        try (InputStreamReader r = new InputStreamReader(
            ...
            cfg = gson.fromJson(r, WindowDecorJson.class);
        } catch (IOException e) {
    ...
    private static void warn(String msg) {
        System.err.println("WindowDecor: " + msg);
    }
}
